...
if __name__ == "__main__":
    ...
    Dict_n = { 
            ...
            'Dict1': {'id': 1, 'location': 'Pizzo Ormea', 'url': 'https://www.eraimperia.it/webcam-ir1dd.php', 'tag': 'img', 'att': "'class': 'src-'", 
            'folder': "PizzoOrmea", 
            'Onedrive': "<img src='https://storage.cloud.google.com/grellag-bucket/PizzoOrmea_ultima.jpg' width='50%'/>"},

            'Dict2': {'id': 2, 'location': "Rifugio Argentea", 'url': 'http://www.comune.arenzano.ge.it/uploads/images/webcam/rifugio/image.jpg', 'tag': 'http', 'att': "", 
            'folder': "Arenzano", 
            'Onedrive': "<img src='https://storage.cloud.google.com/grellag-bucket/Arenzano_ultima.jpg' width='50%'/>"},

            'Dict3': {'id': 3, 'location': "Prato Cipolla", 'url': 'https://retelimet.centrometeoligure.it/stazioni/meteo-cipolla/webcam/current.jpg', 'tag': 'http', 'att': "", 
            'folder': "Cipolla", 
            'Onedrive': "<img src='https://storage.cloud.google.com/grellag-bucket/Cipolla_ultima.jpg' width='50%'/>"},

            'Dict4': {'id': 4, 'location': "Monte Gifarco", 'url': 'https://turbolic.limet.cloud/webcanon.html', 'tag': 'img', 'att': "'class': 'src-'", 
            'folder': "Loco", 
            'Onedrive': "<img src='https://storage.googleapis.com/grellag-bucket/Loco_ultima.jpg' width='50%'/>"}
    ...
    log.debug(f'Found Download folder {download_folder}')
    html_output = "<!DOCTYPE html>"
    for key, p_info in Dict_n.items():
        if type(p_info) is dict:
                download_path = r'C:\Logs\\'
                destination_path = r'C:\Users\giorg\OneDrive\Immagini\webcam\\'
                html_address = p_info['url']
                if is_url_image(p_info['url']): #url è già il percorso immagine 
                    download_file = download_path + '\\' + p_info['folder'] + '\\' + "R" + datetime.today().strftime ("%Y%m%d_%H%M%S") + ".jpg"
                    DownFileFromUrl(p_info['url'], download_path + p_info['folder'], download_file)
                    log.debug(f'Saved {download_file}')
                else:
                    html = get(html_address)
                    soup = Soup(html)
                    #nel TAG img leggo attributo 'src' per il percorso immagine
                    img1 = soup.find('img')
                    if img1 != None:
                        type_return = type(img1)
                    
                        print(f"Soup found a {type_return}")
                        if type_return == list:
                            if len(img1) >= 1:
                                i = 0
                                while i < len(img1):
                        ...
                        #data = soup.find('img', attrs={'class': 'src-'})
                        file_img = img1.attrs['src']
                        if file_img.find("http") == -1 or file_img.find("https") == -1: #devo aggiungere address al path file
                            full_path_img = os.path.join(os.path.dirname(html_address), file_img).replace('\\', '/')
                        else:
                            full_path_img = file_img
                        head, tail = os.path.split(file_img) 
                        download_file = download_path + '\\' + p_info['folder'] + '\\' + tail
                        urlretrieve(full_path_img, download_file)
                log.debug(f'End webscraper: {html_address}')
    
                source = download_path + '\\' + p_info['folder']           # files location
                destination_path  = destination_path + p_info['folder'] 
                new_filename = p_info['folder'] + '_ultima.jpg'
                ...
                #log = logger.logger.setup_applevel_logger(file_name = 'app_list_debug.log')
                folderPath  = destination_path
                files = glob.glob(folderPath + '/**/*',recursive=True)
                viz_df = pd.DataFrame()
                img_cloud = p_info['Onedrive']
                html_output += f"<center>{img_cloud}</center><br>"

                log.debug(f'Aggiornato HTML con immagine: {img_cloud}')
    html_output += "</html>"
    destination = r'C:\Users\giorg\OneDrive\Immagini\webcam' 
    latest_file = "webscraper_webcam__index.html"
    import io
    with io.open(latest_file, "w") as outputfile:
